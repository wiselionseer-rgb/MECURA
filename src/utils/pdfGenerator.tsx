      ...
      <div className="flex flex-col items-center w-[794px]" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
        {guidesToRender.map((guide, gIdx) => (
          <div key={gIdx} className="w-[794px] h-[1123px] bg-white text-[#111827] relative p-12 border border-slate-200 box-border flex flex-col justify-between" style={{ pageBreakAfter: gIdx < guidesToRender.length - 1 ? "always" : "auto" }}>
            {/* Guide Badge */}
            <div className="absolute top-6 right-6 bg-purple-100 text-purple-900 border border-purple-300 font-bold text-[10px] px-2.5 py-0.5 rounded-full uppercase tracking-wider">
              {guide.badge}
            </div>
            ...
            <div>
              ...
              <div className="space-y-6 my-6">
                {guide.items.length === 0 ? (
                  <p className="text-xs text-slate-400 italic m-0">Nenhum produto cadastrado para esta guia.</p>
                ) : (
                    ...
                    return (
                      <div key={idx} className="border-b border-slate-100 pb-5">
                        <div className="flex items-baseline justify-between mb-2">
                          <span className="text-sm font-bold text-slate-900 m-0">
                            {idx + 1}. {item.name}
                          </span>
                          <span className="text-[10px] bg-slate-100 text-slate-700 font-bold px-2 py-0.5 rounded border border-slate-200 m-0">
