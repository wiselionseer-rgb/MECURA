...
<div style="font-family: Arial, sans-serif; color: #000; line-height: 1.6; max-width: 800px; margin: 0 auto; text-align: justify; padding: 20px;" id="agronomic-report-content">
    ...
    <p style="margin-left: 40px; margin-bottom: 15px;">
        ...
        Mensal: [TOTAL_MG_DIA]mg x 30 dias = [TOTAL_MG_MENSAL] mg ou [TOTAL_G_MENSAL]g de CBD/THC/CBG.<br>
...
GERAÇÃO E REGRAS:
Gere APENAS o código HTML validado. Não use markdown \`\`\`html.
Não mude NENHUMA palavra do texto estrutural base, a menos que esteja entre colchetes. Apenas faça a substituição dos campos matemáticos para que fechem perfeitamente com a receita enviada.\`;
`;
