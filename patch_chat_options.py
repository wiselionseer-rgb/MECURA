...
# Update initial options in state
old_initial_state = """      options: [
        "Quem somos nós?",
        "Quais doenças sao tratadas com cannabis",
        "Como funciona a compra e o envio dos medicamentos",
        "Da onde vem o remedio e qual a procedencia",
        "Acesso ao maior banco de sementes internacionais",
        "Como produzir seu roprio medicamento em casa legalizados",
        "Curso de Cultivo do inciante ao Profissional",
        "É legal no Brasil?",
        "Valores e Custos",
        "Falar com um assessor agora"
      ]"""
...
code = code.replace(old_initial_state, new_options)

# Update startOptions array
old_start_options = """      const startOptions = [
        "Quem somos nós?",
        "Quais doenças sao tratadas com cannabis",
        "Como funciona a compra e o envio dos medicamentos",
        "Da onde vem o remedio e qual a procedencia",
...
new_start_options = """      const startOptions = [
        "Quem somos nós?",
        "Quais doenças são tratadas?",
        "Como funciona a compra e envio?",
        "Da onde vêm os medicamentos?",
...
code = code.replace(old_start_options, new_start_options)

# Update if/else conditions to be more flexible, and add "problemas no pagamento"
old_ifs = """      if (lowerText.includes("funciona a compra") || text === "Como funciona a compra e o envio dos medicamentos") {
        ...
        botResponse.text = "Possuímos cursos completos desde o iniciante ao avançado, além de auxiliar no processo de Habeas Corpus para você ter autorização legal para cultivar e produzir seu medicamento no conforto de sua casa.";
      } else if (lowerText.includes("curso")) {
        botResponse.text = "Nosso curso ensina todo o passo a passo do cultivo indoor e outdoor, extração do óleo e como se legalizar. É a melhor forma de se tornar autossuficiente!";
...
new_ifs = """      if (lowerText.includes("funciona a compra") || text === "Como funciona a compra e envio?") {
...
code = code.replace(old_ifs, new_ifs)
