...
// The file has a syntax error at line 341.
// Let's replace everything from `const systemPrompt = \`Você é um Agrônomo` to `const contents = [systemPrompt];`
...
ESTRUTURA OBRIGATÓRIA DO LAUDO (Formato HTML - IDÊNTICO AO DOCUMENTO OFICIAL):
...
GERAÇÃO:
Gere APENAS o código HTML validado, substituindo os dados entre colchetes [ ] pelos dados extraídos da receita médica e laudo médico. Realize os cálculos de forma lógica e completa na parte de dimensionamento.\`;
`;
